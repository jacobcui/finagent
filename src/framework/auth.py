...
def api_register():
    ...
    return (
        jsonify(
            {"message": "Registration successful.", "mock_verify_link": verify_url}
        ),
        201,
    )
...
def register():
    if request.method == "POST":
        ...
        verify_url = url_for("auth.verify_email", token=token, _external=True)
        print(f"\n[MOCK EMAIL] To: {email}")
        print("[MOCK EMAIL] Subject: Verify your account")
        print(f"[MOCK EMAIL] Link: {verify_url}\n")

        flash(
            "Registration successful! "
            "Please check your console for the verification link."
        )
...
def login():
    if request.method == "POST":
        ...
        if not user.is_trial_active:
            flash(
                "Your 3-month trial has expired. Please renew your subscription.",
                "warning",
            )
