...
def generate_pdf(context: dict) -> bytes:
    """
    使用 Jinja2 渲染 HTML 并通过 WeasyPrint 转换为 PDF
    """
...
# ==========================================
def app():
    st.title("🧾 ATO跨境货物贸易税务报表生成器")

    # Sidebar: 配置
    with st.sidebar:
        st.header("1. 纳税人信息配置")
        abn = st.text_input("ABN (Australian Business Number)", value="12 345 678 901")
        company_name = st.text_input("公司名称", value="Example Trading Pty Ltd")
        contact = st.text_input("联系人", value="John Doe")

        st.header("2. 税率配置")
        tax_rate = st.slider("关税税率 (%)", 0.0, 20.0, 5.0, 0.5)
    ...
    # Main: 上传与生成
    st.subheader("3. 数据上传与处理")
    uploaded_file = st.file_uploader("上传交易流水CSV", type=["csv"])

    if uploaded_file:
        try:
            raw_df = pd.read_csv(uploaded_file)
            st.write("原始数据预览:", raw_df.head())

            # 处理数据
            processed_df = process_data(raw_df, tax_rate)

            st.subheader("4. 税额计算结果")
            st.dataframe(
            ...
            c1, c2 = st.columns(2)
            c1.metric("总完税价格 (Total Value)", f"${total_value:,.2f}")
            c2.metric("应纳关税总额 (Total Duty)", f"${total_duty:,.2f}")

            if st.button("生成ATO申报PDF"):
                # 准备渲染上下文
                transactions = []
...
if __name__ == "__main__":
    st.set_page_config(page_title="ATO税务报表生成工具", page_icon="🧾")
    app()
