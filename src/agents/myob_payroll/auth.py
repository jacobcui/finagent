...
class MyobAuth:
    def __init__(self, client_id=None, client_secret=None, redirect_uri=None):
        ...
        self.redirect_uri = redirect_uri or os.getenv(
            "MYOB_REDIRECT_URI", "http://localhost:8080/callback"
        )
        ...
        if not self.client_id or not self.client_secret:
            raise ValueError(
                "MYOB_CLIENT_ID and MYOB_CLIENT_SECRET must be set in "
                "environment variables or passed to constructor."
            )
    ...
    def exchange_code_for_token(self, code):
        ...
        # Note: MYOB token endpoint is actually
        # https://secure.myob.com/oauth2/v1/authorize/
        # but for POST it is standard to use the same base or specific token endpoint.
        # Checking docs: POST https://secure.myob.com/oauth2/v1/authorize/
        # is for getting the token.
